...
%   from L2 components to the Presenter interface.
    ...
    methods (Test)
        ...
        function test_ItemRenameRequested(testCase)
            payload = struct('action','rename', 'oldName','DS1', ...
                'newName','NewDS', 'isParent',true, 'datasetIdx',1, 'colIdx',0);
            testCase.assertEventFires('ItemRenameRequested', ...
                @() notify(testCase.View.TableComp, 'ItemRenameRequested', AppEventData(payload)), ...
                @(p) testCase.verifyEqual(p.newName, 'NewDS'));
        end
